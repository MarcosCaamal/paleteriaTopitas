  ...
  <div class="container-fluid">
    <div class="collapse navbar-collapse" id="myNavbar">
      <ul class="nav navbar-nav">
        ...
        <li class="estMe btns"><a href="ventaD.php">Venta Detalle</a></li>
        <li class="active estMe btns"><a href="compraD.php">Compra Detalle</a></li>
        <li class="estMe btns"><a href="producto.php">Productos</a></li>
        <li class="estMe btns"><a href="ventas.php">Ventas</a></li>
        <li class="estMe btns"><a href="compras.php">Compras</a></li>
...
<div class="container-fluid ">
 ...
 <div class="form-group">
      <label class="control-label tamla" for="nombre">Producto:</label>
      <select id="Tipo" name="idProducto">
 ...
 <div class="form-group">
      <label class="control-label tamla" for="nombre">Compras:</label>
      <select id="Tipo" name="idcompras">
      <option value="0" selected disabled="disabled">Elige una opcion</option>
      <?php $res = $MySQLiconn->query("SELECT * FROM compras");
        while($row=$res->fetch_array())
        {
      ?>
        <option value="<?php echo $row['idcompras']; ?>"><?php echo $row['idcompras']; ?></option>
        <?php
        }
        ?>
      </select>
...
<div class="form-group obtds">
       ...
       <div class="col">
          <input type="text" class="form-control text-uppercase" name="cantidadProducto" placeholder="00"
          required value="<?php if(isset($_GET['editar'])) echo $getROW['cantidadProducto'];  ?>" />
       </div>
    </div>
    <div class="form-group obtds">
 ...
 <?php
  include_once '../Db.php';
  $filtro="select * from detallecompra order by Producto_idProducto";
    $SQL = $MySQLiconn->query($filtro);
?>
 <h3 class="textoTa text-center">Listado de Compra Detalle</h3><BR></BR>
 <table class="table table-hover table-bordered shadow p-3 mb-5 bg-white rounded">
   ...
   while($row=$SQL ->fetch_array())
   {
    ?>
       <tr class="tabCont">
         <td> <?php echo $row['Producto_idProducto']; ?> </td>
         <td> <?php echo $row['compras_idcompras']; ?> </td>
